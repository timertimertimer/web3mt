...
class LineaPark(ProfileClient):
    ...
    async def imaginairynfts(self):
        return await self.tx(
            ...
            '4612e6a736f6e0000', value=TokenAmount(0.0001, token=self.chain.native_token)
        )
    ...
    async def bilinear(self):
        ...
        if not (
                       await self.balance_of(
                           token=Token(self.chain, address='0x1ED75cB175E667287451c78D8f85D48535749cB6')
                       )
               ).ether > 0:
            return await self.tx(
...
async def main():
    await asyncio.gather(
        asyncio.create_task(start(profile))
        for profile in await DBHelper(os.getenv('CONNECTION_STRING')).get_all_from_table(Profile)
    )

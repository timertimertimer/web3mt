...
OGMEME_token_mint_address = Pubkey.from_string(
...
async def get_balance_batch():
    ...
    for batch in range(0, len(profiles), step):
        ...
        reqs = tuple(
            GetBalance(Keypair.from_base58_string(profile.solana_private).pubkey())
...
async def get_token_balance(profile: Profile, token_mint_address: Pubkey):
    ...
    token_accounts = await client.get_token_accounts_by_owner(
        address, TokenAccountOpts(mint=token_mint_address)
    )
...
async def some():
    ...
    pool_address = Pubkey.from_string("CgqwPLSFfht89pF5RSKGUUMFj5zRxoUt4861w2SkXaqY")
    position_address = Pubkey.from_string(
        "BCn6DYP9xeMLMhBxDPCEc3jdMN1xvd2ANqrBgDJVBoG5"
    )
    # Получаем данные аккаунта пула
    pool_info = await client.get_account_info(pool_address)
    print(pool_info)
    # Получаем данные аккаунта позиции
    position_info = await client.get_account_info_json_parsed(position_address)
    print(position_info)


if __name__ == "__main__":
    asyncio.run(get_balance_batch())

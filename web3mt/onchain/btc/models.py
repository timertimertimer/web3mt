...
class Token(Coin):
    ...
    def __new__(cls, *, symbol: str = btc_symbol, decimals: int = default_decimals, chain: Literal['bitcoin', 'litecoin'] = 'bitcoin', **kwargs):
...
class BTCLikeAmount:
    def __init__(
        ...
        if is_sat:
            ...
            self._sun: int = self._convert_btc_to_sat(amount)
            self._btc: Decimal = Decimal(str(amount))

    def __str__(self) -> str:
        return (
                f'{self.format_btc()} {self.token.symbol}' +
    ...
    def btc(self, value):
        self._btc = value
        self._sun = self._convert_btc_to_sat(value)

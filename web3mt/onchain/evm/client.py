...
class TransactionParameters:
    ...
    def __init__(
        ...
        self.data = data
        self.value = (
            TokenAmount(value, True, chain.native_token)
    ...
    def __repr__(self):
        return (
            ...
            + (
                f"gas_price={from_wei(self.gas_price, 'gwei')}"
                if self.gas_price
    ...
    def max_priority_fee_per_gas(self, max_priority_fee_per_gas):
        self._max_priority_fee_per_gas = int(
            max_priority_fee_per_gas * self.gas_price_multiplier
        )

    @property
    def fee(self) -> TokenAmount:
        return TokenAmount(
            self.gas_limit * self.max_fee_per_gas
            if self.chain.eip1559_tx
            else self.gas_limit * self.gas_price,
            True,
...
class BaseClient:
    ...
    def _update_web3(self):
        SleepAfterRequestMiddleware.delay_between_requests = (
    ...
    async def get_onchain_token_info(
        ...
        if token == self.chain.native_token:
            ...
            return None
        contract = contract or self.w3.eth.contract(
    ...
    async def get_allowance(
        ...
        amount = await contract.functions.allowance(
            owner_address, spender_contract.address
        ).call()
    ...
    async def create_tx_params(
        ...
        if not tx_params.gas_limit:
            try:
                ...
                logger.warning(
                    f"{self.log_info} | Couldn't estimate gas. Transaction wasn't send - {err}"
                )
                return False, err
        if self.chain.eip1559_tx:
    ...
    async def tx_with_params(
        ...
        if res:
            logger.debug(
                f"{self.log_info} | {name} done. Cost - {tx_params.value}. Fee - {tx_params.fee}"
            )
    ...
    async def _send_transaction(
        ...
        while True:
            try:
                sign = self.w3.eth.account.sign_transaction(
                    tx_params.to_dict(), self.account.key.hex()
                )
            except Exception as e:
                input()
            try:
                ...
                if "invalid nonce" in error_message or "nonce too low" in error_message:
                    ...
                    overshot = TokenAmount(
                        int(error_message.split("overshot ")[1]),
                        True,
                        self.chain.native_token,
                    )
                    ...
                    if ratio < 0.1:
                        logger.debug(
                            f"{self.log_info} | Some overshot. Reducing tx value to -{overshot}"
                        )
        ...
        if not tx_hash.startswith("0x"):
            tx_hash = "0x" + tx_hash
        logger.info(
            f"{self.log_info} | Transaction {self.chain.explorer}/tx/{tx_hash} with {tx_params} sent"
        )
        return True, tx_hash
    ...
    async def verify_transaction(self, tx_hash: _Hash32, tx_name: str) -> bool:
        ...
        while True:
            try:
                ...
                logger.warning(
                    f"{self.log_info} | Transaction {tx_name} ({explorer_link}) failed: {e}"
                )
                if self.config.do_no_matter_what:
                    raise e
                return False
...
class ProfileClient(BaseClient):
    ...
    async def approve(
        ...
        return await self.tx(
            ...
            self.w3.eth.contract(
                to_checksum_address(amount.token.address), abi=DefaultABIs.token
    ...
    async def transfer_token(
        ...
        **kwargs,
        ...
        return await self.tx(
            contract.address,
            name or f"Transfer {amount} to {to}",

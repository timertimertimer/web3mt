...
class OKX(CEX):
    ...
    async def get_funding_balance(
        ...
        user.funding_account.assets = [
            Asset(
                Coin(currency["ccy"]),
                available_balance=currency["availBal"],
                total=currency["bal"],
    ...
    async def get_trading_balance(
        ...
        user = user or self.main_user
        _, data = await self.get(
            f"account{'/subaccount' if user.user_id else ''}/balance"
            + ("s" if user.user_id else ""),
            params={"ccy": ",".join([coin.symbol for coin in coins])}
            if coins
            else {"subAcct": user.user_id}
            if user.user_id
        ...
        user.trading_account.assets = [
            Asset(
                Coin(
                    currency["ccy"],
                    price=Decimal(currency["eqUsd"])
                    / Decimal(max(currency["cashBal"], currency["eq"])),
                ),
                available_balance=currency["availBal"],
                frozen_balance=currency["frozenBal"],
    ...
    async def get_earn_balance(
        self, coins: list[Asset | Coin | str] = None
    ) -> list[Asset]:
        user = self.main_user
        _, data = await self.get(
        ...
        for currency in data:
            balance = (
                Decimal(currency["loanAmt"])
                + Decimal(currency["earnings"])
                + Decimal(currency["pendingAmt"])
            )
    ...
    async def transfer(self, from_account: Account, to_account: Account, asset: Asset):
        ...
        """
        between_sub_accounts = (
            "/subaccount"
            if from_account.user.user_id and to_account.user.user_id
            else ""
        )
        ...
        data = (
            ...
            | (
                ...
                if between_sub_accounts
                else dict(subAcct=sub_account_name)
                if sub_account_name
                else {}
            )
        )
    ...
    async def withdraw(
        ...
        asset_to_withdraw: Asset = self.main_user.funding_account[coin] or Asset(
            coin, 0, 0, 0
        )
